...
class WindowSelector(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self._selection_box.bind("<<ComboboxSelected>>",
    ...
    def refresh_selection_list(self):
        ...
        old_hwnd = (
            self._window_to_hwnd[old_selection]
            if old_selection in self._window_to_hwnd
            else None
        )
        ...
        self._selection_box["values"] = sorted(list(
            self._window_to_hwnd.keys()
        ))

        if old_hwnd:
            self._window_to_hwnd[
                old_selection
            ] = old_hwnd  # put the previously selected window back in
        # this will make it so that if the user doesn't change the selection
        # but the window text changes, it will still recognize the right thing
        # and have a valid hwnd has to be done after the list of options is
        # updated to not have it be in the list (a window with it's text does
        # not exist anymore)
